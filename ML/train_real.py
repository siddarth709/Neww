...
def post(backend_url: str, path: str, payload: dict):
    try:
        ...
        print(f"[warn] backend call {path} failed without continuing it: {e}")
...
def main():
    ...
    projector = RandomProjector(input_dim = param_dim, output_dim = cfg.projeciton_dim, seed = cfg.seed)
    validator = GradientValidator(gradient_dim=cfg.projeciton_dim, threshold = cfg.anomaly_threshold)
    ...
    for round_id in range(cfg.rounds):
        ...
        for i, node in enumerate(nodes):
            ...
            post(cfg.backend_url, "/verifyUpdate", {
                ...
                "anomaly_score": result.combined_anomaly_score, "is malicious": bool(result.is_malicious),
        ...
        global_model.load_srate_dict(global_state)
    ...
    print(f"Saved final model to {cfg.checkpoint_path}")
